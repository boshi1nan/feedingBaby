...
public class PGSQLitePlugin extends Plugin {
			...
			if (cs != null && cs.moveToFirst()) {
			    ...
			    do {
			    	...
			    	for (int i = 0; i < namesCoint; i++){
			    		...
			    		key = cs.getColumnName(i);
			    		curType = cs.getType(i);
			    		if(android.os.Build.VERSION.SDK_INT >= 11) {
							switch(curType) {
								...
								case Cursor.FIELD_TYPE_INTEGER:
									row.put(key, cs.getInt(i));
									break;
								case Cursor.FIELD_TYPE_FLOAT:
									row.put(key, cs.getFloat(i));
									break;
								case Cursor.FIELD_TYPE_BLOB:
									row.put(key, new String(Base64.encode(cs.getBlob(i), Base64.DEFAULT)));
									break;
								case Cursor.FIELD_TYPE_STRING:
								default: /* (not expected) */
									row.put(key, cs.getString(i));
									break;
							}
						}else{
				    		String name = names[i];
	...
	private PluginResult openDatabese(JSONArray data){
		PluginResult result = null;
		try {
			JSONObject ret = new JSONObject();//返回结果集
	        ...
	        if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED) && !storage.equals(PGSQLitePlugin.USE_INTERNAL) ) {
	        	if (storage.equals(PGSQLitePlugin.USE_EXTERNAL)){
	        		if("".equals(path)){
	        			...
	        			File mapPathFile = new File(path);
